...
1. **Train a model**—Model training includes both training and evaluating the model, as follows: 
  ...
  + **Optimising the model**—For each moodel evaluation, based on evaluation outcoomes you make improvements to your model in order to optimise how well it generalises on non-training data. For instance during data preparation, you might have split your dataset into training, test and validation sets. In cases where you achieve 100% accuracy on your training data but very poor accuracy on test set, you can optimise your model to correct it from memorisation or overfitting if that is the problem. In other instance you may be battling an issue of bigger learning rates thhat fail to converge easily. Through evaluation you can readjust your leaning rate to be neithher too small nor too large for easy convergence of gradient descent. Once your model is free from these learning flaws, it passes for deployment.
...
Machine learning is a continuous cycle\. It is often expected that your model will be applied to data that has been drawn from the same population as what you trained the model with. However, due After deploying a model, you monitor the inferences, collect "ground truth," and evaluate the model to identify drift\. You then increase the accuracy of your inferences by updating your training data to include the newly collected ground truth\. You do this by retraining the model with the new dataset\. As more and more example data becomes available, you continue retraining your model to increase accuracy\.
